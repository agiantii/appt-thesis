\begin{cuzchapter}{系统设计与实现}{chap:design}
    ...
    后端通过NestJS Guard机制实现权限校验，先将\texttt{SlidePermissionGuard}全局注入到NestJS，由
    ...
    而在需要权限校验的接口中，只需按代码段\ref{code:api-persmission-example}所示声明即可。
...
平台支持根据用户描述或幻灯片内容自动生成适配Slidev风格的图像，流程如下\ref{fig:ai-generate-iamge-flowchart}：
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.2\textwidth]{figures/drawio/AI生图流程图.drawio.png}
\end{figure}
...
\end{listing}
    ...
    前端使用@types/diff可视化不同版本的差异，还能一键回滚到任意历史版本，让操作流程更加简单且符合大部分人的直觉。
