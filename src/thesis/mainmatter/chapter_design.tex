\begin{cuzchapter}{系统设计与实现}{chap:design}
    ...
    用户、幻灯片和权限等数据结构较为明确，适合用MySQL这样的关系型数据库管理，不仅管理方便，而且运行速度快、安全可靠性强\cite{lan2004mysql,hu2015mysql}。
    % 主要有以下方面的考量：
    ...
    \subsubsection{具体实现}
    采用AOP，具体代码如\ref{code:checkSlidePermission}
\begin{listing}[H]
    \centering
    \caption{文档权限检查方法}
    \label{code:checkSlidePermission}
    \begin{jscode}
     /**
   * 检查文档权限
   */
  private async checkSlidePermission(
    slideId: number,
    user: any,
    requiredRoles: SlideRole[] | undefined,
    ...
    // 公开文档，可以不需要read权限
    if (slide.isPublic && !requiredPermission?.includes("read")) {
      return true;
    }

    // 检查用户在文档中的角色
    const userRole = await this.slideUserRoleRepo.findOne({
      where: {
        slideId: slideId,
        userId: user.userId,
      },
    ...
    // 确定用户的实际角色（创建者为 owner）
    let actualRole: SlideRole | null = null;
    if (slide.createdBy === user.userId) {
      actualRole = 'owner';
    } else if (userRole) {
      actualRole = userRole.role;
    }

    return this.validatePermission(actualRole,requiredPermission);
  }
    \end{jscode}
\end{listing}
    \subsection{AI辅助功能}
    ...
    系统使用@types/diff可视化不同版本的差异，还能一键回滚到任意历史版本，让操作流程更加简单且符合大部分人的直觉。
    ...
    \subsection{评论功能}
    评论功能模块测试在幻灯片下添加评论和回复评论，如\ref{tab:test-comment}表所示。
