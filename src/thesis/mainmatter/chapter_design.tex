\begin{cuzchapter}{系统设计与实现}{chap:design}
    ...
    \begin{figure}[htbp]
        ...
        \begin{tikzpicture}[
            ...
            layer/.style={rectangle, draw, rounded corners, fill=white, align=center, minimum width=3.5cm, minimum height=1.2cm},
            component/.style={rectangle, draw, fill=white, align=center, minimum width=3cm, minimum height=0.8cm},
        ...
        \node[layer, fill=green!10, below=of frontend] (backend) {后端业务层};
        \node[layer, fill=orange!10, below=of backend] (storage) {数据存储层};

        % 前端组件
        \node[component, fill=blue!5, left=of frontend] (react) {React应用\\React 19 + TypeScript + Vite};
        \node[component, fill=blue!5, right=of frontend] (editor) {CodeMirror 6编辑器\\Markdown语法高亮};
        \node[component, fill=blue!5, above left=0.5cm and 0.5cm of frontend] (yjs) {Yjs客户端\\实时协作};
        \node[component, fill=blue!5, above right=0.5cm and 0.5cm of frontend] (slidev) {Slidev预览组件\\实时预览};

        % 后端组件
        \node[component, fill=green!5, left=of backend] (nest) {NestJS框架\\RESTful API};
        \node[component, fill=green!5, right=of backend] (hocus) {Hocuspocus服务器\\WebSocket协作};
        \node[component, fill=green!5, above left=0.5cm and 0.5cm of backend] (business) {业务模块\\用户/幻灯片/权限管理};
        \node[component, fill=green!5, above right=0.5cm and 0.5cm of backend] (auth) {认证授权\\JWT + RBAC};

        % 存储组件
        \node[component, fill=orange!5, left=of storage] (mysql) {MySQL数据库\\结构化数据存储};
        \node[component, fill=orange!5, right=of storage] (redis) {Redis缓存\\会话与高频数据};
        \node[component, fill=orange!5, below left=0.5cm and 0.5cm of storage] (oss) {对象存储(OSS)\\图片与静态资源};
        ...
        % 标签
        \node[label, above=0.2cm of frontend] {\textbf{用户界面与交互}};
        \node[label, above=0.2cm of backend] {\textbf{业务逻辑处理}};
        \node[label, above=0.2cm of storage] {\textbf{数据持久化存储}};
    ...
    \begin{itemize}
        ...
        \item \textbf{Redis缓存}：内存数据库，用于会话管理和高频数据缓存。
    ...
    \begin{figure}[htbp]
        ...
        \begin{tikzpicture}[
            entity/.style={rectangle, draw, rounded corners, fill=blue!10, align=center, minimum width=2.5cm, minimum height=1cm},
            relationship/.style={diamond, draw, fill=green!10, align=center, minimum width=1.2cm, minimum height=1.2cm},
            arrow/.style={->, >=stealth, thick},
            label/.style={font=\small}
        ]

        % 实体节点
        \node[entity] (user) {用户表\\users};
        \node[entity, right=3cm of user] (space) {幻灯片空间表\\slide\_spaces};
        \node[entity, below=2cm of space] (slide) {幻灯片表\\slides};
        \node[entity, left=3cm of slide] (version) {版本控制表\\slide\_versions};
        \node[entity, right=3cm of slide] (comment) {评论表\\slide\_comments};
        \node[entity, below=2cm of slide] (role) {用户角色表\\slide\_user\_roles};
        \node[entity, left=2cm of role] (snippet) {代码片段表\\snippets};
        \node[entity, right=2cm of role] (regcode) {注册码表\\registration\_codes};

        % 关系节点
        \node[relationship, above right=1cm and 1cm of user] (user_space) {拥有};
        \node[relationship, right=1.5cm of space] (space_slide) {包含};
        \node[relationship, below=1.5cm of slide] (slide_version) {版本化};
        \node[relationship, left=1.5cm of slide] (user_slide) {协作};
        \node[relationship, right=1.5cm of slide] (slide_comment) {评论};
        \node[relationship, below=1cm of user] (user_role) {授权};

        % 连接线
        \draw[arrow] (user) -- node[above, label] {1} (user_space);
        \draw[arrow] (user_space) -- node[above, label] {N} (space);

        \draw[arrow] (space) -- node[above, label] {1} (space_slide);
        \draw[arrow] (space_slide) -- node[above, label] {N} (slide);

        \draw[arrow] (slide) -- node[right, label] {1} (slide_version);
        \draw[arrow] (slide_version) -- node[right, label] {N} (version);

        \draw[arrow] (user) -- node[left, label] {1} (user_slide);
        \draw[arrow] (user_slide) -- node[above, label] {N} (slide);

        \draw[arrow] (slide) -- node[above, label] {1} (slide_comment);
        \draw[arrow] (slide_comment) -- node[above, label] {N} (comment);

        \draw[arrow] (user) -- node[left, label] {1} (user_role);
        \draw[arrow] (user_role) -- node[right, label] {N} (role);

        % 其他实体连接
        \draw[arrow, dashed] (user) -- node[left, label] {创建} (snippet);
        \draw[arrow, dashed] (user) -- node[right, label] {使用} (regcode);

        % 图例
        \node[entity, below right=0.5cm and 0.5cm of role, fill=blue!5, minimum width=2cm] (legend_entity) {实体};
        \node[relationship, right=1cm of legend_entity, fill=green!5, minimum width=1.5cm] (legend_rel) {关系};
