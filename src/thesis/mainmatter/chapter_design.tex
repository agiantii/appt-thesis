...
validatePermission方法的实现如图\ref{fig:validatePermission}所示。
    ...
    本平台将幻灯片文档的内容定义为Yjs中的Y.Text类型，并与CodeMirror编辑器绑定，
    当用户在CodeMirror中编辑内容时，内容的变更会立即转换为Yjs的更新操作，
    % 同时，来自其他用户的远程更新也能实时反映到编辑器界面。
    ...
    % 后端Hocuspocus服务器主要通过代码段\ref{code:Hocuspocus}中的函数实现后端Webocket的权限校验、文档加载与持久化，
    ...
    实时协作前，用户需要在编辑器右上角的协作面板中点击“邀请成员”按钮，输入协作者的用户名（可以通过搜索框搜索用户名）并选择角色权限（owner/editor/commenter/viewer）。
    如图\ref{permission-panel-interface}所示。
    \begin{figure}[htbp]
        \centering
    ...
    协作面板还会实时显示当前在线的协作者列表，包括头像、用户名和编辑状态，如图\ref{fig:shared-doc-presentation-interface}所示。
    ...
    本平台接入通义千问大模型API，能够方便、高效地生成文本内容和图片素材。
    ...
    平台可以根据用户描述生成图像。
    用户需要在内联编辑器中输入图像提示词。
    如\ref{fig:ai-assistant-interface}
    \begin{figure}[htbp]
        \centering
   ...
   实现效果如图\ref{version-control-interface}所示。
  \begin{figure}[htbp]
        \centering
    ...
    slide\_versions通过slide\_id外键关联到slides表，删除幻灯片时，对应的版本记录也会跟着一起删除。
    % 幻灯片的Markdown源码通常不超过数百KB，直接存储完整副本的方案也在
    % 这种全量快照方式虽然比增量存储占用更多空间，
    % 但幻灯片内容通常不超过数百KB，
    % 其优势在于回滚和对比操作无需重建内容，
    % 直接读取目标版本即可，实现简单且可靠。

    在前端界面中，通过@types/diff库可视化当前编辑器内容和所选择版本内容的差异，
    差异部分会以删除和新增的样式渲染。
    \subsection{代码片段管理}

    用户可以很方便地管理和使用自己常用的代码片段，如图\ref{fig:snippets-interface}所示。
    在编辑器中选中代码内容，右键选择“保存为代码片段”，再输入片段名称和描述信息，代码片段就会保存到用户的个人片段库中。
    想要插入代码片段时，点击插入菜单里的“代码片段”选项，就能从片段库中选择需要的片段插入到当前光标位置，还可以在编辑器输入代码片段名称的前缀，触发对应代码提示。
    用户可以在设置页面的代码片段管理界面查看、编辑个人代码片段。
    ...
    \end{figure}
     \subsection{幻灯片空间管理}
    用户可以利用幻灯片空间来组织和管理幻灯片文档。在工作台界面点击对应按钮，输入空间名称并设置可见性，即可创建空间。
    空间里的文档为树形结构，通过拖拽就能调整文档的层级关系，平台也提供了全局搜索功能。

    如图\ref{fig:slideSpace-interface}所示。
        \begin{figure}[htbp]
        \centering
    ...
    平台里集成评论功能，可以对文档内容讨论和反馈，如图\ref{fig:comment-interface}所示。
        \begin{figure}[htbp]
        \centering
