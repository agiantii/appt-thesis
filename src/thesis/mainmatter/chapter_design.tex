...
\begin{figure}[htbp] 
    ...
    \begin{tikzpicture}[
        entity/.style={
            rectangle, draw, minimum width=2.8cm, minimum height=0.9cm, 
            align=center, inner sep=0.2cm, font=\small
        },
        relationship/.style={
            diamond, draw, aspect=2, inner sep=0.1cm, align=center, font=\small
        },
    ...
    \node[entity, right=5cm of user] (space)          {幻灯片空间表\\slide\_spaces};
    \node[entity, below=2.5cm of space] (slide)       {幻灯片表\\slides};
    \node[entity, below=3cm of slide] (version)       {版本控制表\\slide\_versions};
    \node[entity, right=4.5cm of slide] (comment)     {评论表\\slide\_comments};
    \node[entity, below=2.5cm of user] (role)         {用户角色表\\slide\_user\_roles};
    \node[entity, right=3.5cm of role] (snippet)      {代码片段表\\snippets};
    \node[entity, below=2.5cm of snippet] (regcode)   {注册码表\\registration\_codes};

    % ---- 关系菱形节点（置于关联实体中心） ----
    % 用户 - 拥有 - 空间
    \node[relationship] (owns) at ($(user)!0.5!(space)$) {拥有};
    % 空间 - 包含 - 幻灯片
    \node[relationship] (contains) at ($(space)!0.5!(slide)$) {包含};
    % 幻灯片 - 版本化 - 版本
    \node[relationship] (versioned) at ($(slide)!0.5!(version)$) {版本化};
    % 幻灯片 - 评论 - 评论
    \node[relationship] (comments) at ($(slide)!0.5!(comment)$) {评论};
    % 用户 - 授权 - 角色
    \node[relationship] (authorizes) at ($(user)!0.5!(role)$) {授权};
    % 用户 - 协作 - 幻灯片
    \node[relationship] (collaborates) at ($(user)!0.5!(slide) + (-0.5,0)$) {协作};
    % 用户 - 创建 - 代码片段
    \node[relationship] (creates) at ($(user)!0.5!(snippet) + (0,-0.3)$) {创建};
    % 用户 - 使用 - 注册码
    \node[relationship] (uses) at ($(user)!0.5!(regcode) + (0.5,0.3)$) {使用};

    % ---- 连接线与基数标注 ----
    % 用户 -- 拥有 -- 空间 (1:N)
    \draw (user) -- (owns) node[pos=0.3, label] {1};
    \draw (owns) -- (space) node[pos=0.7, label] {N};

    % 空间 -- 包含 -- 幻灯片 (1:N)
    \draw (space) -- (contains) node[pos=0.3, label] {1};
    \draw (contains) -- (slide) node[pos=0.7, label] {N};

    % 幻灯片 -- 版本化 -- 版本 (1:N)
    \draw (slide) -- (versioned) node[pos=0.3, label] {1};
    \draw (versioned) -- (version) node[pos=0.7, label] {N};

    % 幻灯片 -- 评论 -- 评论 (1:N)
    \draw (slide) -- (comments) node[pos=0.3, label] {1};
    \draw (comments) -- (comment) node[pos=0.7, label] {N};

    % 用户 -- 授权 -- 角色 (1:N)
    \draw (user) -- (authorizes) node[pos=0.3, label] {1};
    \draw (authorizes) -- (role) node[pos=0.7, label] {N};

    % 用户 -- 协作 -- 幻灯片 (1:N)
    \draw (user) -- (collaborates) node[pos=0.3, label] {1};
    \draw (collaborates) -- (slide) node[pos=0.7, label] {N};

    % 用户 -- 创建 -- 代码片段 (1:N)
    \draw (user) -- (creates) node[pos=0.3, label] {1};
    \draw (creates) -- (snippet) node[pos=0.7, label] {N};

    % 用户 -- 使用 -- 注册码 (1:N)
    \draw (user) -- (uses) node[pos=0.3, label] {1};
    \draw (uses) -- (regcode) node[pos=0.7, label] {N};

    \end{tikzpicture}
    \caption{APPT平台实体关系图（Chen表示法）}
