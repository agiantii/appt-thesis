\begin{cuzchapter}{系统设计与实现}{chap:design}
    ...
    \subsection{核心表结构}
    % TODO
    本平台的核心数据设计如表 \ref{tab:database-tables} 所示：
    ...
    \begin{table}[htbp]
        ...
        \begin{tabular}{lll}
            ...
            slide\_spaces       & 幻灯片空间表    & 管理幻灯片集合    \\
            slides              & 幻灯片表      & 存储具体的幻灯片文档内容  \\
            slide\_versions     & 版本控制表     & 记录幻灯片的版本历史    \\
            slide\_user\_roles  & 幻灯片的用户角色表     & 定义用户在幻灯片中的角色  \\
            slide\_comments     & 评论表       & 存储对幻灯片的评论和回复  \\
            snippets            & 代码片段表     & 存储用户常用的代码片段   \\
    ...
    评论的树形结构通过reply\_id和物化路径（path）实现，
    ...
    \begin{table}[htbp]
        ...
        \begin{tabular}{llll}
            ...
            path        & varchar(255) & 是                                     & 根节点到本节点的物化路径    \\
    ...
    % reply\_id：reply\_id字段指向同表中父评论的id。
    % 顶层评论的reply\_id为NULL，
    % 子评论的reply\_id指向其直接相邻的父评论。
    % % 邻接表写入成本低，插入新评论只需设置父节点引用，无需修改其他记录，适合高频评论写入场景。
    % 物化路径（path）：path字段存储从根节点到当前节点的完整路径字符串，
    % 如1-5-12。
