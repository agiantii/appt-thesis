\begin{cuzchapter}{系统设计与实现}{chap:design}
    ...
    用户表存储账户信息，包括用户名、密码哈希值、头像URL等主要信息，
    ...
    幻灯片空间表存储幻灯片空间的基本信息，包括空间名称、所有者用户ID等。
    ...
    \begin{table}[htbp]
        ...
        \begin{tabular}{llll}
            ...
            content         & longtext     & 是      & 幻灯片Markdown正文        \\
    ...
    版本控制表记录幻灯片的版本历史，每次保存生成一条新记录，如表\ref{tab:slide-versions}所示。
    ...
    用户角色表定义用户在幻灯片中的角色，实现RBAC权限控制，如表\ref{tab:slide-user-roles}所示。
    ...
    注册码表用于管理平台的注册准入机制，系统管理员可生成注册码并分发给目标用户。
    每个注册码包含有效期和使用状态信息，注册时需校验注册码的有效性和是否已被使用，从而控制用户注册权限。
    ...
    Slidev主题表存储平台可用的主题配置，通过npm包名唯一标识每个主题。
    用户创建幻灯片时可从已有主题中选择应用，系统根据package\_name拉取对应npm包以加载主题样式，
    同时也支持管理员后续扩展更多社区或自定义主题。如表\ref{tab:slidev-themes}所示。
    ...
    Slidev插件表存储平台可用的插件配置，基于npm生态进行扩展。
    每个插件通过package\_name唯一标识，用户在排版幻灯片时可选择启用所需插件以增强演示效果，
    管理员也可根据需求引入新的社区插件。如表\ref{tab:slidev-plugins}所示。
