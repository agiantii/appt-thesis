\begin{cuzchapter}{系统设计与实现}{chap:design}
    ...
    本节主要介绍slides（幻灯片表）与slide\_comments（评论表）两张表的字段设计。

    \subsubsection{slides 幻灯片表}

    幻灯片表存储每个幻灯片文档的标题、内容与状态信息。
    slides表通过parent\_id字段自关联，
    这样，空间内的文档就可以支持任意深度的树形结构。
    并且由于每个幻灯片空间内的文档只能由该空间所有者创建，
    表slides的created\_by字段一定和slide\_spaces表的owner\_id字段相同，
    将created\_by作为冗余字段，可以在部分接口做查询时候减少内联接，增加查询速度。
    ...
    % parent\_id是文档的核心字段，自关联到同表的id，实现任意深度的树形结构；
    % % 根文档的 parent\_id 为 NULL。
    % % 用户可以通过拖拽来调整文档在目录树的位置，而这种变化
    % % 平台支持用户通过拖拽方式调整文档在目录树中的位置，即节点的父子关系可动态变更，
    % % 因此 slides 表仅使用邻接表而不引入物化路径。
    % allow\_comment 与 is\_public 字段用来控制文档的相关权限，
    % is\_build 与 preview\_url 字段分别表示文档是否构建以及构建后的url。
    ...
    % \textbf{reply\_id}：reply\_id字段指向同表中父评论的id。
    % 顶层评论的reply\_id为NULL，
    % 子评论的reply\_id指向其直接相邻的父评论。
    % % 邻接表写入成本低，插入新评论只需设置父节点引用，无需修改其他记录，适合高频评论写入场景。
    % \textbf{物化路径（path）}：path字段存储从根节点到当前节点的完整路径字符串，
    % 如1-5-12。
    % 我们可以使用如SELECT * FROM slide\_comments WHERE path LIKE '1-5-\%'的SQL查询语句，一次性获取任意评论节点下的所有子孙评论，不需要递归查询；
    % 删除某条评论及其所有回复时同样也只需要一条如DELETE FROM slide\_comments WHERE path LIKE '1-5-\%'的SQL语句，
    % 不需要递归删除。
    % \textbf{权限联动控制}：
    % 评论功能受双重权限约束。slides 表中的 allow\_comment 字段由幻灯片所有者控制；
    % 用户角色须为 commenter 及以上级别（commenter、editor、owner）方可发表评论，viewer 角色仅有只读权限。
    ...
    \textbf{为何评论树引入物化路径？}
    path字段存储从根节点到当前节点的完整路径字符串，如1-5-12。
    评论一旦发布，它的结构就不会再发生变化。
    当新增一条评论时，只需要把父评论的path加上当前评论的id，就能得到该评论的path。
    我们可以使用如SELECT * FROM slide\_comments WHERE path LIKE '1-5-\%'的SQL查询语句，一次性获取任意评论节点下的所有子孙评论，不需要递归查询；
    删除某条评论及其所有回复时同样也只需要一条如DELETE FROM slide\_comments WHERE path LIKE '1-5-\%'的SQL语句，不需要递归删除。
