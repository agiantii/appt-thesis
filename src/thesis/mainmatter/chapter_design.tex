...
\usetikzlibrary{calc}
    ...
    用户、幻灯片和权限等数据结构较为明确，适合用关系型数据库管理。本平台选择MySQL\cite{lan2004mysql,hu2015mysql}作为数据库。
    % 主要有以下方面的考量：
...
% TODO
    ...
    \textbf{为何文档树不使用物化路径？} 
    本平台支持用户拖拽来改调整目录树的结构，目录树结构可能会经常更新。
    ...
    以文本编辑为例，假设用户A在位置P插入字符"X"，用户B在相同位置插入字符"Y"。传统方法会产生冲突，
    而CRDT算法通过以下步骤解决：
