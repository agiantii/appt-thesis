...
\usetikzlibrary{calc}
    ...
    本平台采用了前后端分离的设计架构\cite{wu2020frontend}。系统的整体架构如图 \ref{fig:appt-architecture} 所示，包含三个主要层次：前端展示层、后端业务层和数据存储层。
    ...
    \begin{itemize}
        ...
        \item \textbf{服务器状态}：采用React Query管理从服务器获取的数据状态，具备缓存、重试和乐观更新等能力。
    ...
    后端提供RESTful API接口\cite{wang2013restful}，按照资源进行组织：
    ...
    考虑到本平台的数据特点和性能需求，选择MySQL\cite{hu2015mysql,lan2004mysql}作为主要数据库：
...
% TODO
    ...
    \textbf{为何文档树不使用物化路径？} \texttt{slides} 表支持用户通过拖拽调整文档节点的位置，即节点的父子关系会动态变更。若引入物化路径，每次移动一个节点，都需要将该节点及其所有子孙节点的 \texttt{path} 字段全部更新，更新量与子树规模成正比，写放大严重。因此文档树仅维护 \texttt{parent\_id} 邻接表，移动节点只需修改一条记录的父节点引用，代价恒定。删除文档节点时则通过递归遍历依次删除子节点，与文档树规模相对较小、删除操作频率较低的实际使用场景相匹配。
    ...
    本平台采用了基于CRDT（Conflict-Free Replicated Data Type，无冲突复制数据类型）的协同算法来实现实时协作编辑功能\cite{lv2022crdt,algorithms2017crdt}。CRDT作为一种无冲突复制数据类型的数学理论，能够确保在异步网络环境中，多个数据副本之间最终达到一致状态，而不需要中央协调器来解决冲突问题。
    ...
    CRDT算法的数学理论保障了系统的正确性与可靠性，从而为本平台提供了稳定且高效的实时协作体验\cite{sun2019otcrdt,lv2016stringcrdt}。

    \subsubsection{文档协同流程图}

    本平台的文档协同流程\cite{li2020cscw,li1997cscw}采用了条件分支设计方案，能够依据文档的协作状态动态选择独立编辑模式或实时协作模式。具体流程详见图 \ref{fig:collaboration-flowchart}。
    ...
    权限管理系统采用RBAC模型构建\cite{zhang2013rbac,wu2002rbacweb,kuhlmann2013uml}，具体实现包括以下方面：
