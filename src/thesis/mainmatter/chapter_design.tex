...
\begin{figure}[htbp]
    ...
    % 实体节点 - 上层
    \node[entity] (user) at (0, 6) {用户表\\users};
    \node[entity, right=6cm of user] (space) {幻灯片空间表\\slide\_spaces};

    % 实体节点 - 中层
    \node[entity, below=4.5cm of user] (snippet) {代码片段表\\snippets};
    \node[entity, below=4.5cm of space] (slide) {幻灯片表\\slides};
    \node[entity, right=6cm of slide] (comment) {评论表\\slide\_comments};

    % 实体节点 - 下层
    \node[entity, below=4.5cm of snippet] (version) {版本控制表\\slide\_versions};
    \node[entity, below=4.5cm of slide] (role) {用户角色表\\slide\_user\_roles};
    \node[entity, below=4.5cm of comment] (regcode) {注册码表\\registration\_codes};
    ...
    % 标准实线连接，无遮挡
    \draw[arrow] (user) -- node[pos=0.5, left, label] {创建} (snippet);
    \draw[arrow] (user.east) to[out=-30, in=120] node[pos=0.6, right, label] {使用} (regcode.north);
