\begin{cuzchapter}{系统设计与实现}{chap:design}
    ...
    用户、幻灯片和权限等数据结构较为明确，适合用关系型数据库管理，故本平台选择MySQL\cite{lan2004mysql,hu2015mysql}作为数据库。
    % 主要有以下方面的考量：
    ...
    本平台用Yjs库所实现的CRDT\cite{lv2016stringcrdt,algorithms2017crdt}算法实现实时协作编辑，
    幻灯片文档设置为Yjs中的YText，
    ...
    本平台采用了Yjs库实现的基于操作的CRDT算法，
    这个算法处理并发的操作时，不会因为操作的顺序而改变最终的结果，以一个通俗的例子，无论是操作“A”+“B”还是“B”+“A”，其结果都是相同的，
    即使发生网络重传之类等意外情况时，我们可能会意外地执行多个相同的操作，最终的结果也不会受到影响。
