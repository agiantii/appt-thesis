\begin{cuzchapter}{系统设计与实现}{chap:design}
    ...
    具体流程详见图\ref{fig:collaboration-flowchart}。
...
本平台深度集成通义千问大语言模型API，能够高效生成文本内容和图像素材。
不仅支持多轮对话、大纲生成，还支持编辑器内联编辑，
极大提升了Slidev演示文稿的创作效率。
平台特别将Slidev官方文档、主题模板语法规范以及最佳实践案例加入到AI的知识库中，
使AI生成的内容天然符合Slidev的Markdown语法、前端组件使用规范以及主题样式要求。

\subsubsection{大纲智能生成}

% 大纲生成是平台AI辅助的核心入口之一，流程如图\ref{fig:ai-generate-flowchart}所示：
...
% \end{enumerate}
具体实现如代码段\ref{code:ai-outline-generate}所示：
...
\subsubsection{文生图（Text-to-Image）}

平台支持根据用户描述或幻灯片内容自动生成适配Slidev风格的图像，流程如下：
...
% \begin{figure}
%     \centering
%     \includegraphics[options]{figures/drawio/AI生图流程图.drawio.png}
% \end{figure}
