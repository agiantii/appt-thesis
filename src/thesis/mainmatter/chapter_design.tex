\begin{cuzchapter}{系统设计与实现}{chap:design}
    ...
    当新增一条评论时，只需要把父评论的path加上当前评论的id，就能得到该评论的path。开发者可以使用如SELECT * FROM
    ...
    代码片段表用于存储用户创建的自定义代码片段，包括片段名称、片段内容等字段，
    ...
    新增评论时，前端主要向后端传入slideId、userId、replyId、content等参数，对于顶层评论而言，replyId为空，而新增评论的path字段由回复的目标评论的path和自身id组成，
