...
\usetikzlibrary{calc}
    ...
    % 作者希望构建一个功能较为完善且简单易用的智能幻灯片协作平台，具体目标包括：
    % % 作者开发一个功能完善、简单易上手的智能幻灯片协作平台，主要实现以下目标：
    % \textbf{实时协作}：当多用户实时协作编辑时，可以让光标实时追踪；
    % \textbf{权限管理}：基于RBAC模型来设计四级权限管理，来实现文档访问控制；
    % \textbf{AI辅助功能}：用AI辅助生成幻灯片，并提供适配于Slidev语法的智能建议；
    % % \textbf{AI辅助功能}：集成大语言模型API，支持幻灯片内容生成，并提供适配于Slidev语法的智能建议；
    % \textbf{简单的版本控制}：提供版本历史查看、差异对比和历史回滚等功能；


    \subsection{设计原则}

    设计系统时，将遵循模块化设计、前后端分离等原则
    % 在系统设计过程中，遵循以下设计原则：
    ...
    本平台采用了前后端分离的设计架构 。系统的整体架构如图 \ref{fig:appt-architecture} 所示，包含以下三层：前端展示层、后端业务层和数据存储层。
    ...
    前端部分的具体的技术选择如下：
    \textbf{React}：能够高效地完成用户界面渲染与状态管理任务；
    \textbf{React Router}：支持动态路由和嵌套路由；
    \textbf{CodeMirror 6}：能够提供丰富的编辑功能并支持插件系统扩展。


    \subsection{实时协作实现}

    前端的实时协作功能基于Yjs库进行实现，具体的实现方案如下：
    \textbf{文档绑定}：将CodeMirror编辑器与Yjs文档实例进行绑定，从而实现编辑操作的实时同步；
    \textbf{WebSocket连接}：借助@hocuspocus/provider建立与Hocuspocus服务器之间的WebSocket连接；
    ...
    \textbf{模块化架构}：每个模块包含控制器、服务、实体等组件；
    ...
    后端采用Hocuspocus服务器提供WebSocket协作服务：
    \textbf{连接管理}：管理多个客户端的WebSocket连接的建立、维持和断开；
    \textbf{文档同步}：同步Yjs文档的状态变化，确保所有客户端的文档状态保持一致；
    ...
    后端采用多层安全防护机制：
    \textbf{JWT认证}：使用JSON Web Token进行用户身份认证，支持跨域访问和单点登录；
    \textbf{RBAC权限控制}：基于角色的访问控制，定义Owner/Editor/Commenter/Viewer四级权限；
    \textbf{数据验证}：使用自定义的class-validator进行请求参数验证，以防止恶意数据输入；
    \textbf{密码加密}：使用bcrypt算法对用户密码进行哈希存储，确保密码安全。

    \section{数据库设计}\label{sec:database-design}

    \subsection{数据库选型}

    因为用户、幻灯片、权限等数据具有明确的关系结构，适合使用关系型数据库，作者选择MySQL \cite{lan2004mysql,hu2015mysql}作为本平台的数据库。
    % 主要有以下方面的考量：
...
% TODO
    ...
    本节主要介绍 \texttt{slides}（幻灯片表）与 \texttt{slide\_comments}（评论表）两张表的字段设计。两表均以树形结构组织数据，但面对截然不同的使用场景，却采取完全不同的存储策略，具体对比详见本节末尾。
    ...
    \texttt{parent\_id} 是文档的核心字段，自关联至同表的 \texttt{id}，构成邻接表（Adjacency List）；
    ...
    \texttt{allow\_comment} 与 \texttt{is\_public} 字段用于控制文档的相关权限，
    \texttt{is\_build} 与 \texttt{preview\_url} 则记录文档的编译构建状态，分别表示文档是否构建以及构建后的url。


    \subsubsection{slide\_comments 评论表}

    评论的树形结构通过\textbf{邻接表}与\textbf{物化路径}两种模式来实现，
    ...
    评论表的关键设计要点如下：
    \textbf{邻接表（reply\_id）}：\texttt{reply\_id} 字段指向同表中父评论的 \texttt{id}，构成邻接表（Adjacency List）。
    顶层评论的 \texttt{reply\_id} 为 \texttt{NULL}，
    子评论的 \texttt{reply\_id} 指向其直接父节点。
    邻接表写入成本低，插入新评论只需设置父节点引用，无需修改其他记录，
    适合高频评论写入场景。
    \textbf{物化路径（path）}：\texttt{path} 字段存储从根节点到当前节点的完整路径字符串，
    格式如 \texttt{1-5-12}，
    其中每段数字为对应层级评论的 \texttt{id}。
    利用 SQL 前缀查询 \texttt{SELECT * FROM slide\_comments WHERE path LIKE '1-5-\%'}，
    可在不执行递归查询的情况下一次性获取任意评论节点下的全部子孙评论；
    删除某条评论及其所有回复时同样只需一条\texttt{DELETE FROM slide\_comments WHERE path LIKE '1-5-\%'} 语句，
    避免了递归删除带来的多次数据库往返。
    \textbf{权限联动控制}：评论功能受双重权限约束。\texttt{slides} 表中的 \texttt{allow\_comment} 字段由幻灯片所有者控制，决定该文档是否开放评论；用户角色须为 \texttt{commenter} 及以上级别（\texttt{commenter}、\texttt{editor}、\texttt{owner}）方可发表评论，\texttt{viewer} 角色仅有只读权限。
    ...
    实时协作编辑功能基于Yjs CRDT\cite{lv2016stringcrdt,algorithms2017crdt}算法实现，具体实现细节如下：
    \textbf{文档结构设计}：将幻灯片文档设计为Yjs的YText类型，支持富文本编辑和格式标记；
    \textbf{操作同步机制}：通过WebSocket连接同步编辑操作，使用增量更新减少网络传输量；
    \textbf{冲突解决策略}：采用CRDT算法来自动解决并发编辑冲突，保证最终文档的一致性。
