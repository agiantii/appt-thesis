...
\usetikzlibrary{calc}
    ...
    本平台的设计目标在于构建一个功能完善、易用性强的智能幻灯片协作平台，具体目标包括：
    ...
    前端展示层负责用户界面的呈现与交互逻辑的处理，主要包含以下组件：

    \begin{itemize}
        \item \textbf{React应用}：采用React 19、TypeScript和Vite技术栈构建单页应用，具备响应式用户界面能力。

        \item \textbf{CodeMirror 6编辑器}：内置的代码编辑工具，具备Markdown语法高亮和实时预览功能。

        \item \textbf{Yjs客户端}：负责实时协作的客户端组件，通过WebSocket协议与后端协作服务器建立连接。

        \item \textbf{Slidev预览组件}：实现幻灯片实时预览功能的组件，提供了Dev和Build两种预览模式。
    \end{itemize}

    \subsection{后端业务层}

    后端业务层负责业务逻辑处理和数据持久化，主要由以下模块构成：

    \begin{itemize}
        \item \textbf{NestJS框架}：构建在Node.js之上的渐进式框架，提供RESTful API接口。

        \item \textbf{Hocuspocus服务器}：作为WebSocket协作服务器，负责处理Yjs文档的状态同步任务。

        \item \textbf{业务模块}：涵盖用户管理、幻灯片管理、权限控制、AI辅助和版本控制等功能的组件集合。

        \item \textbf{认证授权}：采用JWT认证系统与RBAC权限控制模块实现安全访问。
    \end{itemize}

    \subsection{数据存储层}

    数据存储层负责数据持久化存储，主要包含以下存储组件：

    \begin{itemize}
        \item \textbf{MySQL数据库}：作为关系型数据库，用于存储用户信息、幻灯片内容、权限数据等结构化数据。

        \item \textbf{对象存储}：采用阿里云OSS服务，负责存储上传的图片及其他静态资源文件。
    \end{itemize}
    ...
    前端部分选用了现代化的Web技术栈，具体的技术选择如下：

    \begin{itemize}
        \item \textbf{React 19.2.4}：选用最新的React版本，能够高效地完成用户界面渲染与状态管理任务。

        \item \textbf{TypeScript 5.8}：作为类型安全的JavaScript超集，有助于提升代码质量并提高开发效率。

        \item \textbf{Vite 6.2}：作为下一代前端构建工具，能够提供快速的开发体验并优化生产构建过程。

        \item \textbf{React Router v7}：负责客户端的路由管理工作，支持动态路由和嵌套路由功能。

        \item \textbf{CodeMirror 6}：作为现代化的代码编辑器，提供了丰富的编辑功能并支持插件系统扩展。
    \end{itemize}

    \subsection{组件化设计}

    前端部分采用了组件化的设计模式，将系统功能拆分成可复用的组件模块，具体包括：

    \begin{itemize}
        \item \textbf{布局组件}：例如DashboardLayout、ResizablePanels等，负责页面布局和窗口管理任务。

        \item \textbf{编辑器组件}：包括EditorHeader、EditorSidebar、EditorPreview等，共同实现幻灯片编辑功能。

        \item \textbf{权限组件}：如PermissionModal、CollaboratorModal等，负责管理用户权限和协作成员信息。

        \item \textbf{AI组件}：例如AIPanel、AISuggestion等，用于提供AI辅助功能的用户界面。

        \item \textbf{版本控制组件}：包括VersionHistory、DiffViewer等，负责展示版本历史记录和内容差异信息。
    \end{itemize}

    \subsection{状态管理}

    前端部分采用了分层化的状态管理策略，具体包括：

    \begin{itemize}
        \item \textbf{本地状态}：借助React的useState和useReducer钩子管理组件内部的状态数据。

        \item \textbf{全局状态}：通过Context API管理跨组件的全局状态信息，例如用户认证状态和主题设置等。

        \item \textbf{协作状态}：借助Yjs管理文档的实时协作状态，涵盖文档内容、光标位置和用户状态等信息。
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{文档绑定}：将CodeMirror编辑器与Yjs文档实例进行绑定，从而实现编辑操作的实时同步。

        \item \textbf{WebSocket连接}：借助@hocuspocus/provider建立与Hocuspocus服务器之间的WebSocket连接。

        \item \textbf{光标追踪}：利用Yjs的awareness功能实现实时光标位置显示以及用户状态同步。

        \item \textbf{冲突解决}：通过CRDT算法自动解决并发编辑产生的冲突，从而确保最终的一致性。
    \end{itemize}
    ...
    考虑到本平台的数据特点和性能需求，选择MySQL\cite{lan2004mysql,hu2015mysql} 作为主要数据库：
...
% TODO
    ...
    \begin{itemize}
        ...
        \item \textbf{物化路径（path）}：\texttt{path} 字段存储从根节点到当前节点的完整路径字符串，格式如 \texttt{1-5-12}，其中每段数字为对应层级评论的 \texttt{id}。利用 SQL 前缀查询 \texttt{SELECT * FROM slide\_comments WHERE path LIKE '1-5-\%'}，可在不执行递归查询的情况下一次性获取任意评论节点下的全部子孙评论；删除某条评论及其所有回复时同样只需一条\texttt{DELETE FROM slide\_comments WHERE path LIKE '1-5-\%'} 语句，避免了递归删除带来的多次数据库往返。
    ...
    CRDT算法主要包含以下两种类型：
    \begin{itemize}
        \item \textbf{基于操作的CRDT（Op-based CRDT）}：通过同步操作序列的方式来达成一致性，其中每个操作都具备交换性、结合性和幂等性特征。
        \item \textbf{基于状态的CRDT（State-based CRDT）}：通过同步完整状态或状态差异来实现一致性，状态合并操作需要满足交换律、结合律和幂等律的要求。
    \end{itemize}

    本平台采用了Yjs库实现的基于操作的CRDT算法，该算法具有以下特点：
    \begin{itemize}
        \item \textbf{交换性}：操作的顺序不会影响最终结果，无论操作到达的顺序如何，最终状态都能够保持一致。
        \item \textbf{结合性}：操作组合的方式不会影响最终结果，这为分组传输和批处理优化提供了支持。
        \item \textbf{幂等性}：重复执行相同的操作不会改变状态，从而确保网络重传和重复消息不会引起数据不一致问题。
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Y.Text}：富文本数据类型，支持字符级别的插入、删除和格式属性设置，用于表示幻灯片内容。
        \item \textbf{Y.Array}：有序数组类型，支持元素的插入、删除和移动，用于管理幻灯片页面顺序。
        \item \textbf{Y.Map}：键值对类型，支持属性的增删改查，用于存储幻灯片元数据和用户配置。
    \end{itemize}
    ...
    与传统基于操作转换（OT）的协同算法相比\cite{sun2019otcrdt}，CRDT算法具有以下优势：
    \begin{table}[htbp]
        \caption{CRDT与OT算法对比}
    ...
    \begin{enumerate}
        ...
        \item \textbf{实时协作模式}\cite{cai2021collaborative}：适用于多人协作场景：
        \begin{itemize}
    ...
    这种条件分支设计具有以下优势：
    \begin{itemize}
        \item \textbf{资源优化}：私有文档无需建立WebSocket连接，从而节省服务器资源。
        \item \textbf{性能提升}：独立编辑模式减少了网络延迟，能够提供更快的响应速度。
        \item \textbf{灵活适应}：能够根据实际协作需求动态调整技术方案。
        \item \textbf{用户体验}：保持了编辑界面的一致性，用户无需关注底层技术差异。
    \end{itemize}
    ...
    为确保系统质量，对核心功能进行了黑盒测试。测试重点验证了实时协作、权限管理、AI辅助和版本控制等关键模块的功能完整性。测试结果表明，所有核心功能均正常运行，符合设计要求：

    \begin{itemize}
        \item \textbf{幻灯片创建与编辑模块}：可正常完成幻灯片的新建、标题修改、内容编辑、保存与删除操作，CodeMirror编辑器语法高亮正常，快捷键操作响应及时。

        \item \textbf{Markdown实时预览模块}：编辑器输入内容可实时同步至右侧预览区，Markdown格式（标题、列表、代码块等）渲染正确，无格式错乱。

        \item \textbf {多人实时协作编辑模块}：通过多浏览器标签页模拟多用户协作场景，实现编辑内容实时同步，可正常处理编辑冲突，保证内容不丢失、延迟控制在合理范围。

        \item \textbf{版本管理与回滚模块}：可正常保存版本、查看历史版本记录、对比不同版本差异，一键回滚功能可正常执行，版本数据无丢失。

        \item \textbf{评论与基础权限控制模块}：可对幻灯片内容添加评论、回复评论，角色权限（查看、编辑、评论）分配合理，功能执行正常。

        \item \textbf{AI辅助功能模块}：可正常使用AI辅助功能进行内容生成和优化，生成内容符合Slidev语法规范，响应延迟较低。
    \end{itemize}
