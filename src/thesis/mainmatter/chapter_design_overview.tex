\begin{cuzchapter}{系统概要设计}{chap:design}
    ...
    \section{系统功能结构设计}
    智能幻灯片协作平台主要是为了用户能够方便、高效地创作幻灯片而设计的。
    经过系统需求分析后，可以确定本平台主要实现用户登录、幻灯片管理、幻灯片评论、协作者权限管理、幻灯片创作、代码片段管理等功能模块。
    ...
    % AI辅助功能：用AI辅助演讲者生成幻灯片，并提供适合于Slidev语法的智能建议；
    ...
    is\_public仅用于表明幻灯片空间的基本信息是否公开，当它为1时，并不代表整个幻灯片空间的所有幻灯片公开。
   ...
   \item slide\_comments 评论表
    ...
    评论表存储评论相关信息，包括评论正文内容、所属幻灯片id、评论发表者id、父评论id等字段。
    评论表通过reply\_id和path字段实现评论的树形结构。
    reply\_id指向父评论id，顶层评论的reply\_id为NULL。

    path字段是评论表的物化路径字段，存储从根节点到当前节点的完整路径字符串，如“1-5-12”。评论一旦发布，它的结构就不会再发生变化。
    当新增一条评论时，只需要把父评论的path加上当前评论的id，就能得到该评论的path。作者可以使用如SELECT * FROM slide\_comments WHERE path LIKE '1-5-\%'的SQL查询语句，一次性获取任意评论节点下的所有子孙评论，不需要递归查询；
    删除某条评论及其所有回复时同样也只需要一条如DELETE FROM slide\_comments WHERE path LIKE '1-5-\%'的SQL语句，不需要逐个删除。
    ...
    创建者用户id不仅可以是幻灯片文档的所有者id，也可以是具有编辑权限的协作者的id。
    ...
    用户在编辑幻灯片时，编辑器会根据片段名称前缀检索并显示需要插入的代码片段。
    ...
    Slidev主题表存储平台可用的主题配置，主要包括npm包名、主题名称、预览图URL等字段。
    ...
    唯一的区别就是没有预览图URL，因为插件作为功能性的扩展，无法通过预览图片详细知道使用效果和如何使用。
