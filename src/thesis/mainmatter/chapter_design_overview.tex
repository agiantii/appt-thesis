\begin{cuzchapter}{系统概要设计}{chap:design}
    ...
    \section{系统功能结构设计}
    经过系统需求分析后，
    可以确定本平台主要实现以下功能：平台登录、幻灯片管理、幻灯片评论、协作者权限管理、幻灯片创作、代码片段管理等功能模块。
    系统功能结构图如图\ref{fig:system-function-structure}所示。
    \begin{figure}
        \centering
        \includegraphics[width=0.9\textwidth]{figures/drawio/系统功能结构图.drawio.png}
        \caption{系统功能结构图}
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.9\textwidth]{figures/drawio/核心实体关系图-v3-without-register-code.drawio}
        \caption{核心实体关系图}
    ...
    幻灯片表存储每个幻灯片文档相关信息，包括幻灯片Markdown源码、所属空间ID、编译后预览地址等字段。
    preview\_url是Slidev构建好的幻灯片预览界面链接。
    is\_public字段用于说明幻灯片是否公开，未公开时，只有viewer及以上权限的协作者才能访问该链接。
    ...
    也可以是具有编辑权限的协作者的ID，
    版本控制表结构如表\ref{tab:slide-versions}所示。
