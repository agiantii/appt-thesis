\begin{cuzchapter}{系统测试}{chap:test}
    ...
    \section{多人实时协作编辑功能测试}
    多人实时协作编辑模块利用多开浏览器测试多用户同时编辑时的内容同步等能力，如表\ref{tab:test-collab}所示。

\begin{table}[htbp]
    \caption{多人实时协作编辑功能测试}
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{5.5cm} >{\raggedright\arraybackslash}p{4cm} c}
        ...
        \addlinespace
        撤销操作   & 用户A输入一段文字后按Ctrl+Z撤销，用户B观察编辑区 & B的编辑区同步回滚到撤销后的状态 & 通过 \\
        \bottomrule
    \end{tabular}
\end{table}
    ...
    % 多人实时协作编辑模块：通过多浏览器标签页模拟多用户协作场景，实现编辑内容实时同步，可正常处理编辑冲突，保证内容不丢失、延迟控制在合理范围；
